...
export function TopNav({ onPremiumClick }: { onPremiumClick?: () => void }) {
  ...
  return (
    <div style={{ flexShrink: 0, height: 68, padding: '0 40px 0 20px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', background: '#111111', borderBottom: `1px solid ${T.border}`, zIndex: 30 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 16 }}>
        {/* 汉堡菜单按钮 */}
        <button
          onClick={toggle}
          type="button"
          style={{ width: 44, height: 44, borderRadius: 10, background: 'transparent', border: 'none', color: 'rgba(255,255,255,0.7)', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}
        >
        ...
        {isLoading ? null : isLoggedIn ? (
          <>
            <button
              ...
              type="button"
              style={{ height: 40, padding: '0 18px', borderRadius: 24, border: '1.5px solid', borderColor: 'rgba(139,92,246,0.8)', background: 'linear-gradient(135deg, rgba(45,18,80,0.95) 0%, rgba(25,10,50,0.95) 100%)', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 8, whiteSpace: 'nowrap', boxShadow: '0 0 12px rgba(139,92,246,0.35), inset 0 0 12px rgba(139,92,246,0.08)' }}
            >
              <div style={{ width: 18, height: 18, borderRadius: 4, background: 'linear-gradient(135deg, #a78bfa 0%, #7c3aed 50%, #4c1d95 100%)', boxShadow: '0 0 8px rgba(167,139,250,0.8)', transform: 'rotate(45deg)', flexShrink: 0 }} />
            ...
            <div ref={dropRef} style={{ position: 'relative' }}>
              ...
              {dropOpen && (
                <div style={{ position: 'absolute', top: 52, right: 0, width: 160, background: '#1a1a1a', border: `1px solid ${T.border}`, borderRadius: 12, overflow: 'hidden', zIndex: 100, boxShadow: '0 12px 36px rgba(0,0,0,0.5)' }}>
                  <div onClick={() => { setDropOpen(false); router.push('/profile'); }} style={{ padding: '13px 18px', fontSize: 14, color: '#ffffff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }} onMouseEnter={e => { e.currentTarget.style.color = T.pink; e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; }} onMouseLeave={e => { e.currentTarget.style.color = '#ffffff'; e.currentTarget.style.background = 'transparent'; }}>
                  ...
                  <div onClick={handleLogout} style={{ padding: '13px 18px', fontSize: 14, color: '#ffffff', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 10 }} onMouseEnter={e => { e.currentTarget.style.color = T.pink; e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; }} onMouseLeave={e => { e.currentTarget.style.color = '#ffffff'; e.currentTarget.style.background = 'transparent'; }}>
